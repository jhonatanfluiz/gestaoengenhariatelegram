...
async function askPhaseQuestion(
  ...
  if (phaseNum > 26) {
    log('All phases processed. Ending checklist session.');
    ...
    await sendTelegram('sendMessage', {
      chat_id: chatId,
      text: `🎉 **Checklist Concluído!** Você respondeu a todas as fases da instalação e ajustes. Obrigado por enviar as atualizações!${aiMessage}\n\n📊 [Veja o seu relatório de progresso aqui](${reportUrl})`,
      parse_mode: 'Markdown',
    }, log);
...
async function handleCallbackQuery(callbackQuery: any, log: (...args: any[]) => void) {
  ...
  if (action === 'start') {
    ...
    if (progressPercent === 100) {
      ...
      await sendTelegram('sendMessage', {
        chat_id: chatId,
        text: `✅ **Fase [${phaseNum}/26]: ${phaseName}** concluída com **100%** de progresso!`,
        parse_mode: 'Markdown',
      }, log);
    } else {
      await sendTelegram('sendMessage', {
        chat_id: chatId,
        text: `📈 Progresso da **Fase [${phaseNum}/26]: ${phaseName}** atualizado para **${progressPercent}%** com sucesso. Bom trabalho!`,
        parse_mode: 'Markdown',
      }, log);
    }
